...
$(document).ready(function () {
    $("#tituloPagina").html(`<a class="nav-link active" href="rolPersona">HOTEL | ROLES PERSONA</a>`);
...
$("#filtro").submit(function (e) {
    ...
    $("#tbRolPersona").dataTable().fnDestroy();


    var table = $("#tbRolPersona").DataTable({
        ajax: {
             url: "rolPersona",
            data: {
                estado: $("#activos").val(),
...
$(document).on("click", "#cerrarModal", function () {
    $("#modalRolPersona").modal("hide");
});

$(document).on("click", "#cerrarModalE", function () {
    $("#modalRolPersonaE").modal("hide");
});
    ...
    api.columns()
        ...
        .each(function (colIdx) {
            if (colIdx == 0 || colIdx == 1 ) {
                var cell = $(".filtersRolPersonas th").eq(
                ...
                $(
                    "input",
                    $(".filtersRolPersonas th").eq($(api.column(colIdx).header()).index())
                )
                    .off("keyup change")
...
$("#tbRolPersona thead tr")
    .clone(true)
    .addClass("filtersRolPersonas")
    .appendTo("#tbRolPersona thead");
