...
$("#filtro").submit(function (e) {
    ...
    $("#tbDocAlmacen").dataTable().fnDestroy();
    var table = $("#tbDocAlmacen").DataTable({
        ajax: {
            url: "movAlmacen",
            data: {
...

    {
        ...
        render: function (data, type, full, meta) {
            if (data == "") {
                ...
                return (
                    '<div style="width:50px; margin:auto">' + data + "</div>"
                );
            }
        },
        orderable: false,
...
var butomns = [
    {
        ...
        exportOptions: {
            columns: [0, 1, 3, 4, 5, 6], // las columnas que se exportarán
        },
    },
    {
        extend: "csv",
...
// }
$("#tbDocAlmacen thead tr")
    .clone(true)
    .addClass("filters")
    .appendTo("#tbDocAlmacen thead");
...
$(document).on("click", "#btonNuevo", function () {
    Swal.fire({
        title: "Documento de Almacén del tipo:",
        showConfirmButton: true,
        confirmButtonText: "Ingreso",
        confirmButtonColor: "#0044ff",
        showDenyButton: true,
        denyButtonText: "Egreso",
        denyButtonColor: "#ffc107",
    }).then((result) => {
        if (result.isConfirmed) {
            window.location.href =
                "docAlmacen?operacion=Doc Almacen&tipo=Ingreso";
        } else if (result.isDenied) {
            window.location.href =
                "docAlmacen?operacion=Doc Almacen&tipo=Egreso";
        }
    });
});

function irDetalleCompras(id, operacion) {
    window.location.href = "docAlmacen?id=" + id;
}
